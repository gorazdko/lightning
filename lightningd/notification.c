						...
						const struct failed_htlc *failed)
{
	...
	if (failed->failcode) {
		...
		json_add_num(stream, "failcode", failed->failcode);
		json_add_string(stream, "failreason",
				onion_type_name(failed->failcode));
	}
	else
		json_add_string(stream, "status", "failed");
	if (failed->scid)
		json_add_short_channel_id(stream, "out_channel", failed->scid);
	json_object_end(stream);
}
			 ...
			 const struct failed_htlc *failed)
{
	void (*serialize)(struct json_stream *,
			  const struct short_channel_id *,
			  const struct failed_htlc *) = htlc_failed_notification_gen.serialize;

	struct jsonrpc_notification *n =
	    jsonrpc_notification_start(NULL, "htlc_failed");
	serialize(n->stream, scid, failed);
	jsonrpc_notification_end(n);
